...
let chekValid = false;
function perebro(array){
    for(let i = 0; i<array.length; i++){
        if(array[i].style.color == "rgba(60, 162, 85, 0.9)"){
            chekValid = true;
        }else{
            chekValid = false;
            return;
...
window.addEventListener("load", function(){
    ...
    if (Preregistation) {
        let ArrayMoreInputs = document.querySelectorAll(".regForm input")
        ArrayMoreInputs[0].value = Preregistation.NamePeople;
        ArrayMoreInputs[1].value = Preregistation.SurnamePeople;
        ArrayMoreInputs[2].value = Preregistation.UserName;
        ArrayMoreInputs[3].value = Preregistation.EmailAdres;
        ArrayMoreInputs[4].value = Preregistation.PS;
    }
});

InviteForm.addEventListener("click", function(event){
    event.preventDefault();    
    perebro(errLog);
        if(chekValid == true){
            perebro(errPassoword1);
            if(chekValid == true){
                perebro(errPassoword2);
                if(chekValid == true){
                    let ArrayMoreInputs = document.querySelectorAll(".regForm input")
                    let Preregistation  = {
                        NamePeople: ArrayMoreInputs[0].value,
                        SurnamePeople: ArrayMoreInputs[1].value,
                        UserName: ArrayMoreInputs[2].value,
                        EmailAdres: ArrayMoreInputs[3].value,
                        PS: ArrayMoreInputs[4].value
                    };
